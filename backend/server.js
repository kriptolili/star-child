...
const {
  createStorySpeech,
} = require("./services/speech_service");
...
function createProfileFromRequest(body = {}) {
  ...
  return {
    ...
    appearanceMode:
      typeof appearanceMode === "string"
        ? appearanceMode.trim()
        : "auto",

    appearanceRegion:
      typeof appearanceRegion === "string"
        ? appearanceRegion.trim()
        : "auto",

    storyLanguage:
      typeof storyLanguage === "string"
        ? storyLanguage.trim()
        : "auto",
...
function validateProfile(profile) {
  if (
    ...
    return (
      "Çocuğun adı, doğum tarihi, doğum saati " +
      "ve doğum yeri gereklidir."
    );
  }

  const validAppearanceModes = [
    "auto",
    "custom",
  ];
  ...
  if (
    !validAppearanceRegions.includes(
      profile.appearanceRegion
    )
  ) {
...
app.post("/story", async (req, res) => {
  try {
    ...
    console.log(
      `📖 ${profile.childName} için masal hazırlanıyor...`
    );

    console.log(
      `🌍 Görünüm: ${profile.appearanceMode} / ` +
      `${profile.appearanceRegion}`
    );

    console.log(
      `🌐 Masal dili: ${profile.storyLanguage}`
    );

    console.log(
      `📷 Fotoğraf seçildi: ` +
      `${profile.photoProvided ? "Evet" : "Hayır"}`
    );

    const story =
      await createStoryBookPlan(profile);

    return res.json(story);
  } catch (error) {
    console.error(
      "Masal oluşturma hatası:",
      error
    );
...
app.post("/speech", async (req, res) => {
  try {
    ...
    if (!rawProfile || !story) {
      return res.status(400).json({
        error:
          "Seslendirme için profil ve masal verisi gereklidir.",
      });
    }
    ...
    const speechProfile = {
      ...
      voiceEnabled:
        rawProfile.voiceEnabled !== false,
    };

    if (!speechProfile.childName) {
      return res.status(400).json({
        error: "Çocuğun adı gereklidir.",
      });
    }
    ...
    console.log(
      `🎙️ ${speechProfile.childName} için ` +
    ...
    if (!speechResult) {
      return res.json({
        message:
          "Seslendirme kullanıcı tercihiyle kapatıldı.",
        audioUrl: null,
        fileName: null,
        disclosure:
    ...
    console.error(
      "Seslendirme oluşturma hatası:",
      error
    );

    return res.status(500).json({
      error:
        "Masal seslendirilirken bir sorun oluştu.",
    });
  }
});
...
app.post("/package", async (req, res) => {
  try {
    ...
    if (!rawProfile || !story) {
      return res.status(400).json({
        error:
          "Paket oluşturmak için profil ve masal verisi gereklidir.",
      });
    }
    ...
    console.log(
      `📦 ${profile.childName} için ` +
      "Star Child paketi hazırlanıyor..."
    );

    const packageResult =
      await createStarPackage({
        profile,
        story,
      });

    console.log(
      `✅ Star Child paketi hazır: ` +
    ...
    console.error(
      "Star Child paketi oluşturma hatası:",
      error
    );

    return res.status(500).json({
      error:
        "Resimler, PDF, ses ve video hazırlanırken " +
        "bir sorun oluştu.",
    });
  }
});
